...
def locale_from_request(request) -> str:
    ...
    for quality, _index, language in sorted(candidates, reverse=True):
        ...
        if any(item.split("-", 1)[0].lower() == requested_primary for item in SUPPORTED_LOCALES):
            return normalized
    # 管理员在后台设置中保存的默认语言作为最后回退。
    try:
        from . import storage as _storage  # 延迟导入避免循环依赖

        configured_default = str(_storage.get_settings().get("ui_language") or "").strip()
        if configured_default:
            return normalize_locale(configured_default)
    except Exception:
        pass
    return DEFAULT_LOCALE
