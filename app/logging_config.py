...
# 除 C0/DEL 外还剥离 Unicode 格式控制符（bidi 覆盖/零宽字符），防日志视觉层伪造。
...
def _tail_log_snapshot(max_lines: int = 300) -> tuple[list[str], str, dict[str, object]]:
    ...
    block_size = 65536
    # Keep chunks in reverse file order while scanning.  Repeatedly prepending
    # bytes (`chunk + data`) copies the whole accumulated tail on every read;
    # joining once at the end keeps the bounded tail read linear in its size.
    chunks: list[bytes] = []
    delimiter_count = 0
    bytes_read = 0
    position = 0
    ...
    try:
        with LOG_FILE.open("rb") as handle:
            ...
            while position > 0 and bytes_read < max_read:
                record_count = delimiter_count
                if chunks and chunks[0][-1] not in {0x0D, 0x0A}:
                    record_count += 1
                if record_count > limit:
                    break
                read_size = min(block_size, position, max_read - bytes_read)
                if read_size <= 0:
                    break
                position -= read_size
                ...
                if not chunk:
                    break
                chunk_delimiters = chunk.count(b"\r") + chunk.count(b"\n") - chunk.count(b"\r\n")
                if chunks and chunk.endswith(b"\r") and chunks[-1].startswith(b"\n"):
                    # CRLF can straddle two blocks read in reverse order.
                    chunk_delimiters -= 1
                delimiter_count += chunk_delimiters
                chunks.append(chunk)
...
def runtime_log_facets(entries: list[dict[str, object]]) -> dict[str, list[dict[str, object]]]:
    """Build bounded, safe facets for the admin runtime-log filters.

    Runtime logs come from more than one formatter, so the UI cannot rely on a
    single field name. Keep this normalization on the server as well as in the
    adapter: it makes the endpoint useful to non-browser clients and prevents
    the filter controls from depending on an arbitrary page-size sample.
    """
    ...
    device_keys = ("device_id", "device", "device_ref", "deviceId")
    user_keys = ("username", "user", "user_id", "userId", "actor")

    def add(kind: str, value: object) -> None:
        text = sanitize_log_text(value, 160).strip()
        if not text or text in {"—", "-", "none", "null", "<redacted>"}:
            return
    ...
    for entry in entries[-2000:]:
        if not isinstance(entry, dict):
            continue
        add("loggers", entry.get("logger"))
        add("events", entry.get("event_name"))
        attrs = entry.get("attributes")
        if not isinstance(attrs, dict):
            attrs = {}
        for key in device_keys:
            if attrs.get(key) not in (None, ""):
                add("devices", attrs.get(key))
                break
        for key in user_keys:
            if attrs.get(key) not in (None, ""):
                add("users", attrs.get(key))
                break

    def values(kind: str, label_key: str) -> list[dict[str, object]]:
        rows = counters[kind]
        return [
            {"id": value, label_key: value, "count": count}
            for value, count in sorted(rows.items(), key=lambda item: (-item[1], item[0]))[:200]
        ]

    return {
        "loggers": values("loggers", "name"),
        "events": values("events", "name"),
        "devices": values("devices", "name"),
        "users": values("users", "username"),
    }
...
def runtime_log_snapshot(
    ...
    if severity_filter:
        # 显式严重级别筛选保留最大读取窗口, 保证筛选完整性。
        tail_limit = 2000
    else:
        # 默认视图只读取 limit 与 600 行的较大者, 避免固定解析 2000 行
        # 造成每次日志请求数百毫秒的固定开销。
        tail_limit = max(limit, 600)
    raw_lines, scanned_raw_text, tail_meta = _tail_log_snapshot(tail_limit)
    ...
    meta.update(
        {
            ...
            "facets": runtime_log_facets(returned),
        }
    )
