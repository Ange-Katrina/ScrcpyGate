...
管理员宫格上方列出全部可发现的 ALAS 配置，包括未绑定设备的配置，以迷你胶囊显示名称和当前任务。绿色表示运行，灰色表示空闲或停止，红色表示检查失败，橙色表示等待或过期；悬停及辅助技术可读取完整状态，不显示检查时间。在宫格可见时每 30 秒查询管理员总览并绕过浏览器读取缓存；隐藏标签页或离开宫格后暂停。检查失败与正常停止分别显示。

点击设备卡片上的活动曲线按钮，展开 CPU 使用率、内存、CPU 温度三行数据，再次点击可收起。展开后每 15 秒通过共用串行队列刷新；标签页隐藏时暂停，离开宫格、设备移除或撤权后停止。CPU 为一秒间隔的全核利用率；Android 禁止读取 `/proc/stat` 时，尝试 `dumpsys cpuinfo`，并明确标注为系统最近统计。内存按 `MemTotal - MemAvailable` 计算，不是单个应用的占用。结果（包括指标不可用）缓存十秒，每个应用进程最多两台设备并发采样，CPU/内存 ADB 超时四秒；可选温度查询另设两秒超时，温度失败保留 CPU/内存结果。不支持的指标显示未知，ADB 连接失败与超时分别提示。CPU 温度优先读取 `dumpsys thermalservice` 的当前 CPU 类型传感器，后备读取 `/sys/class/thermal/thermal_zone*/` 中明确命名为 CPU 的可读节点；多个 CPU 传感器取最高读数。不使用缓存、热保护阈值或电池、GPU、机身温度替代；无传感器或权限不足显示不可用，无需电池。数值按绿/橙/红区分，CPU 分界为 60/85%，内存为 70/90%，CPU 温度为 70/85°C；这是视觉参考分级，不是设备专属热保护阈值，未知值保持中性色。采样不启动投屏、不获取控制权，无需增加部署配置或依赖。

宫格、用户卡片和安全表单统一紧凑控件尺寸，并保留触屏点击区域。安全机制沿用管理后台的标题与面板布局，其分区标签及 ALAS 标签采用共享分段控件尺寸。地区库下载配置在桌面将来源、下载内容与连接设置分组并排，限制字段宽度；手机上单列展示。已保存的代理地址继续明文显示，原生文件选择框与其他上传控件对齐。
...
发送按键、文字或触摸操作前，先获取设备控制权。**更多 → 文本输入 → 实体键盘输入方式**提供两种模式：

- **本机输入法（默认）**：使用电脑输入法选出中文。中文和表情通过设备剪贴板粘贴，手机当前输入框必须支持粘贴。单次文字提交最多 4096 个 UTF-8 字节；超过限制会提示，不会截断后发送。
- **设备输入法**：像 QtScrcpy 的普通键盘路径一样，将实体键盘按键发给 Android。电脑切换为英文输入，手机使用兼容的输入法，在手机端选出中文。此模式需要桌面实体键盘。

在画面上右击可返回或唤醒设备，中键点击可回到主页。**更多**还提供音量、电源键以及明确的关闭/点亮屏幕命令。**关闭设备屏幕**用于请求关闭实体屏幕并继续投屏，支持情况取决于设备。「指令已发送」只表示已传输，请确认设备实际响应。

旋转按钮只改变本地画面；**更多 → 恢复自动方向**恢复自动适配，不修改 Android 的旋转设置。滚轮和触摸坐标随显示方向转换；切换方向或离开浏览器时，会在控制通道可用的情况下释放正在进行的手势。
...
预设卡片使用 720p、1080p 等直观档位名称。这些是尺寸上限档位，实际编码尺寸仍保持设备比例，并不固定为标准宽高。
...
**用户与权限**采用可搜索、筛选的用户卡片。点击卡片可查看登录 IP、上次登录、累计投屏时长、投屏次数、最近开始/结束及当前连接。最近设备记录和登录记录默认收起；展开登录记录时读取审计保留期内最近最多 20 条成功登录。编辑与权限按钮保留在卡片上，重置密码和删除位于详情窗口。
...
地区库面板按 Country / City 显示库大小、构建时间、更新时间、来源、启用状态及备份占用。服务器无法连接下载服务时，可在 **手动上传地区库** 选择类型，上传从 MaxMind 获取的 `.mmdb`（最大 256 MiB）或官方 `.tar.gz`（最大 128 MiB），再点击 **校验并启用**。此入口仅管理员可用，包含 CSRF 校验和审计，不需要下载凭据；如经过反向代理，请允许对应请求体大小及最长 10 分钟上传时间。类型不符、损坏、超过 30 天或早于当前版本的上传会被拒绝。新库通过校验后才替换，激活或状态保存失败恢复原库，每种类型最多保留一份 `.mmdb.bak`；相同内容不重复替换或新增备份。

在线更新联合比较远端 `Last-Modified`、文件大小和强 ETag / `Content-MD5`，同时重新验证本地库类型、有效期、大小及 SHA-256；所有证据一致才跳过下载。ETag 只作为版本标识，不当作 MD5。服务器提供 `Content-MD5` 时会验证下载内容，同时验证声明大小和 MMDB。旧版本缺少校验记录或远端信息不足时会下载一次进行确认，但相同内容不会重复落盘；临时上传和解压文件会在结束后清理。
...
**删除库**需确认后才删除所选 Country / City 主文件及备份，也可清理仅剩备份的条目。操作仅限管理员并校验 CSRF，与下载、上传共用锁。强制执行模式下禁止删除当前使用的库；确需删除时，先切换观察或关闭模式并保存。下载选项保持不变，仍勾选的库会在后续更新时重新下载。状态保存失败会恢复原文件；恢复或清理失败会明确提示，并保留 `.recovery` 文件供手动恢复。删除不清除凭据，也不改变访问策略。

国家预设、可编辑的 ISO 代码与提示文字分别显示独立标签和控件，保存后才生效。
