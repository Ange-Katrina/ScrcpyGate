...
点击设备卡片上的活动曲线按钮，展开 CPU、内存、电池温度三行数据，再次点击可收起。展开后每 15 秒通过共用串行队列刷新；标签页隐藏时暂停，离开宫格、设备移除或撤权后停止。CPU 为一秒间隔的全核利用率；Android 禁止读取 `/proc/stat` 时，尝试 `dumpsys cpuinfo`，并明确标注为系统最近统计。内存按 `MemTotal - MemAvailable` 计算，不是单个应用的占用。结果（包括指标不可用）缓存十秒，每个应用进程最多两台设备并发采样，ADB 超时四秒。不支持的指标显示未知，ADB 连接失败与超时分别提示。温度来自 Android `dumpsys battery` 的十分之一摄氏度读数，明确标注为电池温度，不代表 CPU 温度；未提供时显示不可用。数值按绿/橙/红区分，CPU 分界为 60/85%，内存为 70/90%，电池温度为 40/45°C；这是视觉参考分级，不是设备专属热保护阈值，未知值保持中性色。采样不启动投屏、不获取控制权，无需增加部署配置或依赖。
