...
The initial password is shown once and is not stored in plaintext under `data/`. If it is lost, use the `reset-admin` command below. Installation failures return a non-zero status and include container status and recent logs.
