...
每轮交付更新都必须递增 `VERSION`，包括 `dev` 分支更新和源码部署包，按上表
选择版本变化。同一轮可以包含多个提交；中间修改、CI 重试、同一源码重新打包
不重复递增。后续交付不得降低或复用之前的版本号。

交付提交或推送前，同步版本号、英文更新说明和必要的升级步骤；从验证后的
提交更新 `ScrcpyGate-deploy`。源码中的版本号不代表该版本已经发布到 GitHub
Release 或 GHCR。
...
1. 确认本轮交付已递增 `VERSION`，完成验证和英文更新说明；正式发布 PR 沿用该版本。
