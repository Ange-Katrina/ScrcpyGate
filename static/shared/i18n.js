...
(function () {
  ...
  var EN = {
    ...
    "更新状态": "Update status",
    "更新规则与帮助": "Update rules and help",
    "读取中…": "Loading…",
    "有未保存修改": "Unsaved changes",
    "配置不完整": "Configuration incomplete",
    "留空保留现有值": "Leave blank to keep the current value",
    "填写 License Key": "Enter License Key",
    "下载权限尚未验证": "Download access not yet verified",
    "保存选项并检查更新": "Save options and check",
    "重试更新": "Retry update",
    "保存或提交中…": "Saving or submitting…",
    "检查时会先保存当前下载选项。": "Your current download options will be saved before checking.",
    "已就绪，可以检查更新。": "Ready to check for updates.",
    "可重试倒计时：": "Retry in: ",
    "请先保存已填写的下载凭据，再检查更新。": "Save the credentials you entered before checking for updates.",
    "City 包含国家、省份与城市，通常只需选择 City；下载成功前继续使用现有地区库。": "City includes country, region and city data and is usually sufficient. The current database stays active until the download succeeds.",
    "留空保留已保存的凭据；更换 Account ID 时须同时填写新 Key。": "Leave fields blank to keep saved credentials. A different Account ID requires a new Key.",
    "保存只校验格式，检查更新时验证下载权限。Key 不回显。": "Saving checks the format. Checking for updates verifies download access. The Key is never displayed.",
    "自动检查间隔可设为 1–168 小时，保存后立即生效。": "Set an automatic check interval of 1–168 hours. Saving applies it immediately.",
    "成功检查且配置未变时，间隔 10 分钟；失败重试或修改配置后，距上次尝试满 30 秒即可重试。每天最多尝试 30 次。": "Unchanged settings after a successful check require 10 minutes between checks. Failed attempts or changed settings can retry 30 seconds after the previous attempt. Limit: 30 attempts per day.",
    "凭据保存在私有数据卷，保留数据重装会保留配置。下载权限状态仅代表最近检查结果，不是网页登录状态，无法查询 Key 到期日。": "Credentials stay in the private data volume and survive reinstalls that retain data. Access status reflects the last check, not a web login session; Key expiration cannot be queried.",
    "IP 地域是网络出口估算，可能受代理或移动网络影响。请先使用「只观察」预演，再启用强制执行。": "Geolocation estimates the network exit and may be affected by proxies or mobile networks. Preview in Observe mode before enforcing.",
    "· 本产品使用": "· This product uses",
    "数据。": "data.",
    "正在短暂冷却，请按倒计时重试。": "A brief cooldown is active. Retry when the countdown finishes.",
    "下载连接或传输超时，原有地区库保留；可检查代理后重试。": "The download connection or transfer timed out. The current database is retained; check the proxy and retry.",
    "无法解析下载服务器域名，请检查服务器 DNS。": "The download hostname could not be resolved. Check the server DNS.",
    "HTTPS 证书校验失败，请检查服务器时间、CA 证书和代理。": "HTTPS certificate verification failed. Check the server clock, CA certificates and proxy.",
    "下载连接被拒绝，请检查服务器出站规则和代理端口。": "The download connection was refused. Check outbound rules and the proxy port.",
    "地区库文件下载被拒绝，请重试或检查代理；这不代表 Key 无效。": "The database file download was denied. Retry or check the proxy; this does not mean the Key is invalid.",
    "MaxMind 暂时限制下载频率，请稍后重试。": "MaxMind temporarily rate-limited downloads. Retry later.",
    "下载服务暂时不可用，请稍后重试。": "The download service is temporarily unavailable. Retry later.",
    "下载服务返回异常状态，请查看运行记录后重试。": "The download service returned an unexpected status. Check the run log and retry.",
    "地区库文件操作失败，请检查数据目录权限和剩余磁盘空间。": "A database file operation failed. Check data-directory permissions and free disk space.",
    "下载重定向地址未通过安全校验，请检查代理或更新服务版本。": "The download redirect failed security validation. Check the proxy or update the service.",
    "导出完整日志": "Export all retained logs",
    "导出全部保留的审计、告警和运行日志，不受筛选或分页影响": "Export all retained audit records, alerts and runtime logs, regardless of filters or pagination",
    "正在导出…": "Exporting…",
    "正在整理全部保留的日志，请稍候": "Preparing all retained logs. Please wait.",
    "完整日志已生成，开始下载 ZIP": "Full log archive is ready. Downloading ZIP.",
    "审计写入尚未完成，请稍后重新导出": "Audit writes are still pending. Please retry shortly.",
    "已有完整日志正在导出，请稍后重试": "A full log export is already running. Please retry shortly.",
    "日志超过 LOG_EXPORT_MAX_BYTES 限制，未生成不完整文件；请调整限额后重试": "Logs exceed LOG_EXPORT_MAX_BYTES. No partial archive was created. Adjust the limit and retry.",
    "完整日志导出超时，未生成不完整文件": "Full log export timed out. No partial archive was created.",
    "运行日志正在轮转，请重新导出": "Runtime logs changed during rotation. Please retry the export.",
    "运行日志文件不可读取，请检查文件类型与权限": "Runtime logs cannot be read. Check file types and permissions.",
    "完整日志导出失败，请稍后重试": "Full log export failed. Please retry shortly.",
    "完整日志导出失败，请检查日志目录权限与临时磁盘空间": "Full log export failed. Check log directory permissions and temporary disk space.",
    "下载响应格式不正确，请检查登录状态或网关": "Unexpected download format. Check your session or gateway.",
    "选择需要下载的国家库或城市库，使用同一组 MaxMind 凭据。City 已包含国家信息；通常只需启用 City。": "Choose country or city downloads using the same MaxMind credentials. City includes country data and is usually all you need.",
    "下载内容": "Databases to download",
    "国家归属，下载较小": "Country attribution, smaller download",
    "国家、省份与城市（推荐）": "Country, region and city (recommended)",
    "关闭某项不会删除已有数据库；全部关闭将暂停地区库下载，过期后仍按地域策略处理。": "Disabling a database keeps its existing file. Disabling both pauses downloads; expired databases still follow your region policy.",
    "下载连接": "Download connection",
    "跟随服务器代理": "Use server proxy settings",
    "直接连接": "Direct connection",
    "自定义代理": "Custom proxy",
    "HTTP / HTTPS 代理地址": "HTTP / HTTPS proxy URL",
    "地址须包含端口，可使用 http://用户名:密码@主机:端口；特殊字符需 URL 编码。保存后保持显示，仅在选择自定义代理时使用。": "Include a port, for example http://user:password@host:port. URL-encode special characters. The saved URL remains visible and is used only in Custom proxy mode.",
    "清除已保存的代理地址": "Clear the saved proxy URL",
    "保存下载选项": "Save download options",
    "下载选项已保存，下次检查时生效。": "Download options saved. They apply at the next check.",
    "代理地址已保存，可直接修改。": "The proxy URL is saved and can be edited directly.",
    "尚未保存自定义代理地址。": "No custom proxy URL is saved.",
    "下载运行记录": "Download activity",
    "跟随最新日志": "Follow latest entries",
    "仅保留本次任务最近 120 条记录；重启服务或更改下载配置后清空。不显示密钥或代理地址。": "Shows the latest 120 entries for this task. Restarting the service or changing download settings clears them. Keys and proxy URLs are never shown.",
    "暂无运行记录，点击「立即检查更新」后可查看下载过程。": "No activity yet. Select Check for updates to view the download process.",
    "该数据库已更新": "Database updated",
    "该数据库已是最新版本": "Database already current",
    "本次检查完成": "Check completed",
    "已暂停所有地区库下载，请先选择下载内容并保存。": "All database downloads are paused. Select a database and save first.",
    "请选择 Country 或 City。": "Select Country or City.",
    "下载选项格式不正确。": "Invalid download options.",
    "无法读取下载配置，请重新保存或检查文件权限。": "Cannot read download settings. Save them again or check file permissions.",
    "下载配置文件权限不安全，请设为仅服务账户可读写（0600）。": "Download settings have unsafe permissions. Restrict them to the service account (0600).",
    "保存下载配置失败，请检查数据目录权限与磁盘空间。": "Could not save download settings. Check data directory permissions and free space.",
    "代理地址无效，请填写包含端口的 HTTP/HTTPS 地址，不要填写下载镜像链接。": "Enter an HTTP/HTTPS proxy URL with a port. Download mirror URLs are not supported.",
    "请选择有效的下载连接方式。": "Select a valid download connection mode.",
    "请填写代理地址，或选择直接连接。": "Enter a proxy URL or select Direct connection.",
    "无法使用代理隧道，请检查代理地址、认证信息及 CONNECT 支持。": "Could not use the proxy tunnel. Check the URL, credentials and CONNECT support.",
    "连接暂时中断，正在自动重试；显示的是上次获取的进度。": "Connection interrupted. Retrying automatically; the last received progress is shown.",
    "登录已失效或无权读取状态，请重新登录或检查访问权限。": "Your session expired or access was denied. Sign in again or check access permissions.",
    "状态刷新已暂停，请点击「刷新」重试；后台更新可能仍在运行。": "Status refresh paused. Select Refresh to retry; the background update may still be running.",
    "下载的数据库类型不匹配，原有地区库保留。": "The downloaded database type does not match. The existing database was retained.",
    "地区库超过下载大小限制，原有地区库保留。": "The database exceeds the download size limit. The existing database was retained.",
    "定位精度": "Location detail",
    "排队中…": "Queued",
    "正在验证下载凭据…": "Verifying download credentials",
    "正在下载地区库…": "Downloading the geolocation database",
    "正在校验地区库…": "Validating the geolocation database",
    "正在启用新地区库…": "Activating the new database",
    "正在清理旧库…": "Cleaning up old databases",
    "本次更新失败": "This update failed",
    "正在更新地区库…": "Updating the geolocation database",
    "国家 / 省份 / 城市": "Country / Region / City",
    "仅国家": "Country only",
    "当前仍使用国家库。点击「立即检查更新」下载城市库后，归属将显示可用的省市信息。": "A country database is currently active. Select Check for updates to download the city database and show available regions and cities.",
    "归属按当前地区库显示国家、省份和城市，缺失时显示已有信息。国家筛选、明细与导出保留访问当时的记录；内网地址无法定位。IP 位置仅为网络出口估算。": "Attribution shows countries, regions and cities from the current database, with a fallback when details are missing. Country filters, details and exports retain the original records. Private addresses cannot be geolocated. Locations estimate the network exit only.",
    "自动下载 GeoLite2 City 城市库，使用现有 Account ID 和 License Key。旧国家库在城市库成功下载前继续使用；城市库体积较大，首次更新可能较慢。": "GeoLite2 City downloads use your existing Account ID and License Key. The old country database remains active until the city download succeeds. City databases are larger; the first update may take longer.",

    "国家或地区预设": "Country or region preset",
    "自定义国家/地区": "Custom countries / regions",
    "中国大陆（CN）": "Mainland China (CN)",
    "中国大陆、香港、澳门、台湾": "Mainland China, Hong Kong, Macao, Taiwan",
    "中国及日本、韩国、新加坡": "China, Japan, South Korea, Singapore",
    "预设仅填入代码，点击「保存地域设置」后生效。": "Presets fill in codes only. Select Save geo settings to apply.",
    "正在读取凭据状态…": "Loading credential status\u2026",
    "使用 Account ID 和 License Key 验证下载权限，无网页登录会话，也无法查询账户或 Key 的到期日。状态仅代表最近一次检查结果。": "Download access uses an Account ID and License Key, not a web login session. Account and key expiry dates are unavailable. Status reflects the most recent check only.",
    "首次配置请填写两项。保存只验证格式；点击「立即检查更新」后，将通过 MaxMind 官方下载端点验证权限并更新地区库。": "Enter both fields for initial setup. Saving checks the format only. Check for updates verifies access through the official MaxMind download endpoint and updates the database.",
    "自动检查间隔（小时）": "Automatic check interval (hours)",
    "保存更新周期": "Save update interval",
    "可设为 1–168 小时，推荐 12 小时。后台设置优先于环境变量默认值，保存后立即重新安排检查；会附加随机延迟以错开下载高峰。": "Choose 1\u2013168 hours; 12 hours is recommended. This setting overrides the environment default and reschedules checks immediately. Random delay spreads download traffic.",
    "手动检查至少间隔 10 分钟，每日最多尝试 30 次。地区库不可用时，强制执行会拒绝访问。": "Manual checks require a 10-minute interval, with at most 30 attempts per day. Enforce mode denies access when the database is unavailable.",
    "更新周期必须为 1–168 小时的整数。": "The update interval must be a whole number from 1 to 168 hours.",
    "未配置下载凭据": "Download credentials not configured",
    "凭据已配置 · 待验证": "Credentials configured \u00b7 Not yet verified",
    "下载权限已验证": "Download access verified",
    "凭据被拒绝 · 请检查或更换 Key": "Credentials rejected \u00b7 Check or replace the key",
    "暂时无法验证 · 请检查网络后重试": "Unable to verify now \u00b7 Check connectivity and retry",
    "最近检查：": "Last check: ",
    "最近验证通过：": "Last verified: ",
    "更新周期已保存，无需重启。": "Update interval saved. No restart required.",
    "归属按当前地区库查询；国家筛选、明细与导出保留访问当时的记录。内网地址无法定位国家。": "Attribution uses the current database. Country filters, details and exports retain the original records. Private addresses cannot be geolocated.",
    "内网地址": "Private address",
    "地区库不可用": "Country database unavailable",
    "无法定位": "Unknown location",

    "地区库错误：": "Database issue: ",
    "（强制执行时会按保护策略拒绝请求）。": " (Enforce mode denies requests when the database is unavailable).",
    "尚未下载地区库。": "The country database has not been downloaded.",
    "地区库已过期，请检查自动更新。": "The database has expired. Check automatic updates.",
    "无法读取地区库，请检查文件和权限。": "Cannot read the database. Check the file and permissions.",
    "下载的地区库校验失败，旧库未替换。": "Database validation failed. The previous database was retained.",
    "缺少地区库读取组件，请更新完整镜像。": "The database reader dependency is missing. Install an up-to-date complete image.",
    "地区库查询失败，请检查库状态。": "Database lookup failed. Check database status.",
    // GeoIP credentials and in-place security controls.
    "配置 MaxMind 下载凭据，不需要提供账户登录密码。留空保留已保存的值；更换 Account ID 时须同时填写新 Key。": "Configure MaxMind download credentials; no account password is needed. Leave fields blank to keep saved values. Changing the Account ID requires a new Key.",
    "填写新 Key；留空保留": "Enter a new Key, or leave blank to keep it",
    "保存下载凭据": "Save download credentials",
    "清除已保存凭据": "Clear saved credentials",
    "获取 License Key": "Get a License Key",
    "保存凭据后，默认每 12 小时自动检查（附加随机延迟）；也可立即检查。手动检查至少间隔 10 分钟，每日最多尝试 30 次。地区库不可用时，强制执行会拒绝访问。": "After saving credentials, check automatically every 12 hours by default with jitter, or check manually. Checks are at least 10 minutes apart, with up to 30 attempts per day. Enforce mode denies access if the database is unavailable.",
    "IP 地域是估算结果，代理、VPN 和移动网络可能造成偏差。建议先使用「只观察」并预演当前来源，再启用强制执行。": "IP geolocation is an estimate; proxies, VPNs and mobile networks can affect accuracy. Start in Observe mode and preview your current source before enforcing.",
    "MaxMind 更新文档": "MaxMind update documentation",
    "· 凭据保存在私有数据卷，保留数据重装会保留配置；清除凭据不会删除现有地区库或取消地域策略。": "· Credentials are stored in the private data volume and survive reinstalls that retain data. Clearing credentials does not remove the database or disable the policy.",
    "轻量 · 12–14 bits": "Light · 12–14 bits",
    "均衡 · 14–18 bits": "Balanced · 14–18 bits",
    "加强 · 16–20 bits": "Stronger · 16–20 bits",
    "PoW 难度预设": "PoW difficulty presets",
    "以上预设只调整计算难度。每增加 1 bit，期望计算量约翻倍；建议先试算，再用手机验证登录体验。点击「保存登录设置」后对新挑战生效。": "These presets change difficulty only. Each additional bit approximately doubles the expected work. Benchmark first, then test sign-in on a phone. Save login settings to apply to new challenges.",
    "首题难度不能高于难度上限。": "The first challenge difficulty cannot exceed the ceiling.",
    "高难度可能让手机验证超过 30 秒计算期限；建议降低上限并实测。": "High difficulty may exceed the 30-second compute deadline on phones. Lower the ceiling and test on your devices.",
    "难度阶梯": "Difficulty progression",
    "确认并封禁": "Confirm ban",
    "封禁 IP": "Ban IP",
    "调整封禁期限": "Change ban duration",
    "选择期限并填写原因，确认后立即生效。": "Choose a duration and enter a reason. Changes take effect on confirmation.",
    "确认后会立即断开该地址的现有连接。若封禁自己的来源，将失去访问权限；可在服务器执行 ./deploy.sh --unban IP 恢复。本机与可信代理地址受保护。": "Confirmation immediately disconnects this address. Banning your own source blocks your access; run ./deploy.sh --unban IP on the server to recover. Local and trusted proxy addresses are protected.",
    "无法读取封禁状态，请重试。": "Cannot load ban status. Please retry.",
    "封禁已生效，访问记录已刷新。": "Ban applied. Access records refreshed.",
    "已封禁当前来源。请在服务器执行 ./deploy.sh --unban IP 恢复访问。": "Your current source is banned. Run ./deploy.sh --unban IP on the server to restore access.",
    "请先配置 License Key。": "Configure a License Key first.",
    "请先配置 Account ID。": "Configure an Account ID first.",
    "填写账户数字编号，不是邮箱或用户名；粘贴时会去除首尾空白。": "Enter the numeric account ID, not an email or username. Leading and trailing whitespace is removed.",
    "使用同一账户生成的完整 Key，不是登录密码或仅显示的前四位。": "Use the full Key generated by the same account, not your password or the four-character preview.",
    "首次配置请填写两项。保存只验证格式；保存后点击「立即检查更新」，下载成功才表示凭据和 GeoLite2 Country 下载权限可用。": "Enter both fields for initial setup. Saving checks the format only; use Check for updates to verify credentials and GeoLite2 Country download access.",
    "凭据已配置，可点击「立即检查更新」验证下载权限。": "Credentials are configured. Use Check for updates to verify download access.",
    "Account ID 必须为数字（最多 20 位）。": "Account ID must contain up to 20 digits.",
    "请填写有效的 License Key（最多 256 个字符）。": "Enter a valid License Key (up to 256 characters).",
    "更换 Account ID 时请同时填写新 Key。": "Enter a new Key when changing the Account ID.",
    "凭据格式不正确。": "Invalid credential format.",
    "凭据由环境变量管理，请在服务器修改或移除环境变量后重启。": "Credentials are managed by environment variables. Change or remove them on the server and restart.",
    "无法读取已保存凭据。请重新填写两项或清除后配置。": "Cannot read saved credentials. Re-enter both fields or clear and configure them again.",
    "凭据文件权限不安全，请设为仅服务账户可读写（0600）。": "Unsafe credential file permissions. Restrict access to the service account (0600).",
    "保存失败，请检查数据目录写入权限与剩余空间。": "Save failed. Check data directory permissions and free space.",
    "MaxMind 拒绝了凭据，请确认 Account ID、Key 和数据库下载权限。": "MaxMind rejected the credentials. Check the Account ID, Key and database download permission.",
    "无法连接 MaxMind，请检查服务器 HTTPS 出站网络和代理设置。": "Cannot reach MaxMind. Check outbound HTTPS connectivity and proxy settings.",
    "下载超时，请稍后重试。": "Download timed out. Please retry later.",
    "检查过于频繁，请等待 10 分钟冷却结束。": "Checks are too frequent. Wait for the 10-minute cooldown.",
    "已达到今日 30 次尝试上限，请明日重试。": "The daily limit of 30 attempts has been reached. Retry tomorrow.",
    "更新正在进行，请完成后再修改凭据。": "An update is running. Wait for it to finish before changing credentials.",
    "自动更新已由服务器关闭（GEO_UPDATE_ENABLED=false）。": "Updates are disabled by the server (GEO_UPDATE_ENABLED=false).",
    "当前使用服务器环境变量，后台无法覆盖。清除仅删除后台保存的副本。": "Using server environment credentials, which this page cannot override. Clear only removes the saved copy.",
    "已保存下载凭据。Key 不会回显，留空保留。": "Download credentials are saved. The Key is never displayed; leave blank to keep it.",
    "尚未配置下载凭据。": "Download credentials are not configured.",
    "清除下载凭据": "Clear download credentials",
    "清除后台保存的 Account ID 和 Key？": "Clear the saved Account ID and Key?",
    "地区库与地域策略保留。环境变量中的凭据不受影响。": "The database and regional policy remain. Environment credentials are unaffected.",
    "清除凭据": "Clear credentials",
    "已清除后台保存的凭据。": "Saved credentials cleared.",
    "凭据已保存，可点击「立即检查更新」验证下载权限。": "Credentials saved. Use Check for updates to verify download access.",
    // Administration workspace: compact secondary settings and clearer task copy.
    '这些限制保护管理员工具和模拟器设置，仅对普通用户生效；管理员仍可访问完整页面。': 'These restrictions protect administrative tools and emulator settings for standard users. Administrators retain full access.',
    '审计事件': 'Audit events',
    '高风险事件': 'High-risk events',
    '画质档位': 'Quality presets',
    '实时记录 · 按需刷新': 'Live records · Refresh on demand',
    '设备列表，可横向滚动': 'Device list, scroll horizontally',
    '投屏会话，可横向滚动': 'Viewing sessions, scroll horizontally',
    '用户列表，可横向滚动': 'User list, scroll horizontally',
    '日志列表，可横向滚动': 'Log list, scroll horizontally',
    '横向滚动查看完整列与操作': 'Scroll horizontally to see all columns and actions',
    '查看普通用户不可访问的页面与任务': 'Review pages and tasks restricted for standard users',
    '以下限制由服务器统一管理，此处仅供查看。按任务分组展示普通用户不可见的内容。': 'These restrictions are managed by the server and are read-only here. Content hidden from standard users is grouped by task.',
    '标记为受限的页面与功能对普通用户隐藏，直接访问也会被拒绝。': 'Restricted pages and features are hidden from standard users. Direct access is also denied.',
    '服务器管理 · 只读': 'Server managed · Read-only',
    '按角色设置状态条、控制按钮与全屏显示': 'Set status indicators, controls and fullscreen behavior for each role',
    '先选择角色，再调整工作台。这里仅设置界面显示，访问权限请在「用户与权限」中管理。': 'Choose a role, then customize its workbench. These settings control appearance. Manage access in Users and permissions.',
    '从设备列表选择设备后，点击「开始观看」': 'Select a device from the device list, then choose Start viewing',
    // Security workspace: task-based navigation and progressive disclosure.
    '更新封禁': 'Update ban',
    '确认并封禁': 'Confirm ban',
    '服务器已临时关闭地域限制。这里保存的策略暂不执行。': 'Region rules are temporarily disabled on the server. Saved policies are not enforced.',
    '地区库不可用，当前按保护策略拒绝访问。请恢复地区库或从服务器关闭地域限制。': 'The GeoIP database is unavailable and access is currently denied. Restore the database or disable region rules on the server.',
    '地区库尚未就绪。请展开「地区库与自动更新」完成配置，再开启地域限制。': 'The GeoIP database is not ready. Open Database and updates to set it up before enabling region rules.',
    '查看访问情况，管理来源限制与登录保护。': 'Review visits, manage access rules and protect sign-ins.',
    '安全管理分区': 'Security sections', '操作审计': 'Activity audit',
    '采集与保留设置': 'Collection and retention',
    '这里的更改自动保存。关闭采集不会停止 IP 封禁或地域限制。': 'Changes here save automatically. Disabling collection does not disable IP bans or region rules.',
    '包含未登录访客；不记录查询参数、正文或 Cookie。': 'Includes signed-out visitors. Query strings, request bodies and cookies are not recorded.',
    '过期记录自动清理；选择不清理时仍受总行数上限保护。': 'Expired records are removed automatically. The row limit still applies when age-based cleanup is off.',
    '更多筛选': 'More filters', '清除筛选': 'Clear filters', 'IPv4 或 IPv6 地址': 'IPv4 or IPv6 address',
    '按来源 IP 汇总。选择明细查看请求，或直接管理封禁。': 'Grouped by source IP. View requests or manage a ban directly.',
    '筛选已修改，点击「应用筛选」更新结果。': 'Filters changed. Select Apply filters to update the results.',
    '当前结果已应用筛选。': 'Filters applied to the current results.',
    '表格可横向滚动，查看其余列。': 'Scroll the table horizontally to see more columns.',
    '访问记录表格': 'Access records table', '封禁记录表格': 'IP bans table',
    '新建封禁': 'New ban', '确认 IP 封禁': 'Confirm IP ban', '解除 IP 封禁': 'Lift IP ban',
    '封禁会立即断开该地址的现有连接，到期后自动恢复访问。': 'Banning disconnects this IP immediately. Access is restored when the ban expires.',
    '本机与可信代理地址受保护。提交前将再次确认目标和时长。': 'Local and trusted proxy addresses are protected. Review the target and duration before confirming.',
    '封禁列表；开启「显示已解除与已过期」可查历史。': 'IP bans. Enable Show revoked and expired to view history.',
    '现有连接将断开。如果这是你当前的来源，你也会失去访问权限；可在服务器执行': 'Existing connections will close. If this is your current source, you will also lose access. On the server, run',
    '按国家或地区管理访问。建议先只观察，再预演自己的来源，最后开启强制执行。': 'Manage access by country or region. Observe first, preview your own IP, then enforce the policy.',
    '地区库与自动更新': 'GeoIP database and updates', '保存前预演': 'Preview before saving',
    '使用当前表单检查访问结果，不会保存或改变策略。': 'Check the current form without saving or changing the active policy.',
    '设置只在点击保存后生效。': 'Changes take effect only after saving.',
    '自动更新需在服务器配置 MaxMind Account ID 和 License Key。地区库不可用时，强制执行会拒绝访问。': 'Automatic updates require a MaxMind account ID and license key on the server. Enforce mode denies access when the database is unavailable.',
    '本产品使用 GeoLite2 数据，来源：': 'This product uses GeoLite2 data from',
    '高级登录设置': 'Advanced sign-in settings', '自动登录限制': 'Automatic sign-in restrictions', '保存登录设置': 'Save sign-in settings',
    '只管理登录验证与失败限制。访问记录、IP 封禁和地域限制分别设置，不受登录保护开关影响。': 'Controls sign-in verification and failed attempts only. Access records, IP bans and region rules are configured independently.',
    '已保留请求': 'Retained requests', '国家/地区代码': 'Country/region code', '精确状态码': 'Exact status code',
    '封禁状态': 'Ban status', '未封禁': 'Not banned', '永久封禁': 'Permanently banned', '封禁至': 'Banned until',
    '改期': 'Change expiry', '已解除': 'Revoked', '已到期': 'Expired', '地区库不可用': 'GeoIP database unavailable',
    '地区库更新任务运行中…': 'GeoIP update running…', '地区库更新任务已提交': 'GeoIP update queued',
    '地区库已是最新版本': 'GeoIP database is already current',
    '连接已停止，请检查访问权限或稍后重试。': 'Connection stopped. Check your access or try again later.',
    '访问已被拒绝，请检查访问权限': 'Access denied. Check your access permissions.',
    '确认封禁': 'Confirm ban for', '，时长：': ', duration: ',
    '？现有连接将断开。如果这是你当前的来源，你也会失去访问权限；可在服务器执行': '? Existing connections will close. If this is your current source, you will also lose access. On the server, run',
    '恢复。': 'to recover.',
    ...
    // Login protection: explicit, self-hosted PoW verification.
    '点击开始验证': 'Click to verify',
    '正在验证…': 'Verifying…',
    '验证完成': 'Verified',
    '验证在本机完成，无需拖动或识图。': 'Verification runs locally. No dragging or image puzzles.',
    '请稍候，正在完成安全验证。': 'Please wait while verification completes.',
    '请先输入用户名，再点击验证。': 'Enter your username before starting verification.',
    '验证已过期，请重新验证。': 'Verification expired. Please verify again.',
    '验证耗时过长，请重试。': 'Verification timed out. Please retry.',
    '验证请求过于频繁，请稍后重试。': 'Too many verification requests. Please wait and retry.',
    '验证服务暂不可用，请重试。': 'Verification is temporarily unavailable. Please retry.',
    '请先点击验证按钮，完成验证后再登录。': 'Click the verification button before signing in.',
    '暂时无法验证': 'Verification unavailable',
    '内置 PoW': 'Built-in PoW',
    '验证提供方': 'Verification provider',
    '通过部署配置选择，切换后需重启。': 'Selected in deployment configuration; restart after switching.',
    '本机试算仅对应内置 PoW，不代表其他提供方的性能。': 'This benchmark measures built-in PoW only, not other providers.',
    '准备验证': 'Preparing verification',
    '验证会自动完成，无需拖动滑块。': 'Verification runs automatically. No slider required.',
    '重新验证': 'Retry verification',
    '请输入用户名后自动验证': 'Enter your username to start verification',
    '正在验证，请稍候…': 'Verifying, please wait…',
    '验证完成，可以登录': 'Verified. You can sign in.',
    '验证未完成，请重试。': 'Verification did not complete. Please retry.',
    '验证失败，请重试。': 'Verification failed. Please retry.',
    '验证组件加载失败，请刷新页面重试。': 'Verification could not load. Refresh the page to retry.',
    '人机验证需要 HTTPS；本地测试请使用 localhost 或 SSH 隧道。': 'Verification requires HTTPS. For local tests, use localhost or an SSH tunnel.',
    '登录已暂时封禁，请稍后重试。': 'Sign-in is temporarily blocked. Please try again later.',
    ...
    // 管理面板 · 工作量证明难度（PoW）
    '工作量证明难度': 'Proof-of-work difficulty', '首题难度': 'First challenge', '失败递增': 'Escalation per failure',
    '难度上限': 'Difficulty ceiling', '本机试算': 'Local benchmark',
    '触发人机验证后第一道题的难度；每 +1 表示期望计算量翻倍': 'Difficulty of the first challenge after verification is required; each +1 doubles the expected work',
    '每多失败一次追加的难度；设为 0 表示所有题目同难度': 'Difficulty added per further failure; 0 keeps every challenge at the same level',
    '阶梯封顶值；越高越能挡住脚本，但慢设备登录也会更久': 'Ceiling of the ladder; higher blocks scripts harder but also slows down slow devices',
    '按期望计算量估算；点右侧按钮用本机浏览器把难度换算成真实耗时': 'Estimated from the expected work; press the button to convert the difficulty into real time on this browser',
    '开始试算': 'Run benchmark', '试算中…': 'Benchmarking…', '重新试算': 'Run again',
    '首题工作量证明难度（bits）': 'First-challenge proof-of-work difficulty (bits)',
    '每次失败追加的难度（bits）': 'Difficulty added per failure (bits)',
    '工作量证明难度上限（bits）': 'Proof-of-work difficulty ceiling (bits)',
    // 管理面板 · 访问记录（VIS）
    '访问记录': 'Access records', '记录访问': 'Record access', '记录保留时长': 'Record retention',
    '来源 IP': 'Source IP', '归属': 'Origin', '请求': 'Requests', '拦截': 'Denied',
    '最近': 'Latest', '首次': 'First seen', '操作': 'Actions', '时间': 'Time', '方法': 'Method', '路径': 'Path',
    '耗时': 'Duration', '客户端': 'Client', '明细': 'Details', '关闭明细': 'Close details',
    '刷新': 'Refresh', '导出 CSV': 'Export CSV', '导出 JSON': 'Export JSON',
    '应用筛选': 'Apply filters', '包含后台自动轮询': 'Include background polling',
    '全部': 'All', '仅 4xx': '4xx only', '仅 5xx': '5xx only',
    '已登录账户': 'Signed-in accounts', '未登录': 'Anonymous', '身份': 'Identity', '类型': 'Kind',
    '页面': 'Page', '接口': 'API', '静态资源': 'Static asset', '长连接': 'WebSocket',
    '时间范围': 'Time range', '最近 1 小时': 'Last hour', '最近 6 小时': 'Last 6 hours',
    '最近 24 小时': 'Last 24 hours', '最近 7 天': 'Last 7 days', '最近 31 天': 'Last 31 days',
    '请求总数': 'Total requests', '独立来源': 'Distinct sources', '被拒绝': 'Denied',
    '健康检查（不计入访客）': 'Health probes (not visitors)',
    '可疑访问线索': 'Suspected-scan hints', '加载更多来源': 'Load more sources', '加载更多记录': 'Load more records',
    '记录中': 'Recording', '已停止记录': 'Recording stopped', '记录不完整': 'Incomplete records',
    '该时间范围内没有访问记录': 'No access records in this time range',
    '没有明细记录': 'No detail records',
    '加载访问记录失败': 'Failed to load access records', '访问记录保留': 'Access records kept for',
    '记录在写入前已脱敏：不含查询参数、正文、Cookie 与 Referer；路径中的长随机串会被折叠。': 'Records are sanitized before writing: no query strings, bodies, cookies or Referer; long random path segments are folded.',
    '按来源 IP 汇总；点「明细」查看该地址的逐条记录': 'Summarized per source IP; use "Details" for the individual records of one address',
    '记录每次到达本站的请求（来源 IP、路径、状态码、登录身份）；不记录查询参数、正文与 Cookie': 'Record every request that reaches this service (source IP, path, status, signed-in identity); query strings, bodies and cookies are never recorded',
    '超期记录每天清理一次；「不清理」只关闭按天清理，总行数上限仍然生效': 'Expired records are cleaned once a day; "Do not clean up" only disables age-based cleanup, the row cap still applies',
    '访问记录操作': 'Access record actions', '访问记录筛选': 'Access record filters', '访问统计': 'Access statistics',
    '按来源 IP 筛选': 'Filter by source IP', '按状态筛选': 'Filter by status', '按身份筛选': 'Filter by identity',
    '按请求类型筛选': 'Filter by request kind', '按时间范围筛选': 'Filter by time range',
    '访问记录保留天数': 'Access record retention in days',
    '按身份筛选明细': 'Filter details by identity', '按请求类型筛选明细': 'Filter details by request kind',
    // 管理面板 · IP 封禁（BAN）
    'IP 封禁': 'IP bans', '时长': 'Duration', '期限': 'Expires',
    '原因（可选）': 'Reason (optional)', '操作者': 'Actor',
    '封禁该地址': 'Ban this address', '当前没有封禁': 'No active bans', '永久': 'Permanent',
    '已到期': 'Expired', '已失效': 'Inactive', '事件': 'Events', '关闭': 'Close',
    '显示已解除与已过期': 'Show lifted and expired', '生效中': 'Active', '今日新增': 'Added today',
    '历史累计': 'All time', '事件条数': 'Events', '15 分钟': '15 minutes', '1 小时': '1 hour',
    '24 小时': '24 hours', '7 天': '7 days', '自定义…': 'Custom…', '自定义时长': 'Custom duration',
    '自定义时长必须大于 0 分钟': 'Custom duration must be more than 0 minutes',
    '请先填写来源 IP': 'Enter a source IP first', '解除失败': 'Unblock failed',
    '已解除': 'Lifted', '封禁失败': 'Ban failed',
    '生效中的封禁；解除后该地址可立即重新访问': 'Active bans; a lifted address can access again immediately',
    '封禁按来源 IP 生效，覆盖页面、接口、静态资源与投屏长连接；封禁后会立即断开该地址已有的连接，其他观看端不受影响。环回、本机与可信代理地址不允许封禁，/healthz 永不受影响。': 'Bans apply per source IP and cover pages, APIs, static assets and mirror WebSockets; existing connections from that address are closed immediately while other viewers are unaffected. Loopback, local and trusted-proxy addresses cannot be banned, and /healthz is never affected.',
    '该地址是环回、本机或可信代理地址，封禁它会连健康探针或反向代理一起挡掉，因此被拒绝。': 'That address is loopback, local or a trusted proxy; banning it would also cut off the health probe or reverse proxy, so it was refused.',
    '要封禁的来源 IP': 'Source IP to ban', '封禁时长预设': 'Ban duration preset',
    '自定义封禁分钟数': 'Custom ban duration in minutes', '封禁原因': 'Ban reason', '封禁计数': 'Ban counters',
    '封禁操作': 'Ban actions', '封禁事件流': 'Ban event stream', '新建封禁': 'New ban',
    '的封禁？该地址将立即可以访问。': '? The address will be able to access again immediately.',
    // 管理面板 · 地域限制（GEO）
    '地域限制': 'Region restriction', '地域限制模式': 'Region mode', '不启用': 'Off',
    '只观察': 'Observe only', '强制执行': 'Enforce', '允许的国家/地区': 'Allowed countries/regions',
    '无法定位来源': 'Unlocatable sources', '拒绝': 'Deny', '放行': 'Allow', '例外网段': 'Exception networks',
    '预演': 'Preview', '预演来源': 'Preview source', '留空 = 我当前的来源': 'Empty = my current source',
    '立即检查更新': 'Check for updates now', '保存地域设置': 'Save region settings',
    '地区库': 'Database', '可用': 'Available', '不可用': 'Unavailable', '库版本': 'Database built',
    '库大小': 'Database size', '上次成功': 'Last success', '今日下载': 'Downloads today',
    '下次自动检查': 'Next automatic check', '地区库状态': 'Region database status',
    '地域设置已保存并立即生效': 'Region settings saved and applied',
    '地域限制操作': 'Region restriction actions', '地域预演': 'Region preview',
    '保存地域设置组': 'Save region settings', '地域限制模式选择': 'Region mode',
    '无法定位来源的处理方式': 'Handling of unlocatable sources',
    '允许的国家或地区代码': 'Allowed country or region codes', '例外网段 CIDR 列表': 'Exception CIDR list',
    '预演用的来源地址': 'Source address to preview',
    '两位 ISO 码，逗号分隔；HK、MO、TW 与 CN 彼此独立（只写 CN 不会放行港澳台）': 'Two-letter ISO codes, comma separated; HK, MO and TW are independent from CN (listing CN does not allow them)',
    '查不到国家时如何处理；内网地址也走这一条，局域网部署请配合下面的例外网段': 'What happens when the country cannot be resolved; private addresses also take this path, so LAN deployments should use the exception networks below',
    'CIDR，逗号分隔；这些地址不受地域限制（仍然受 IP 封禁约束，封禁优先）': 'CIDR list, comma separated; these addresses bypass the region check (IP bans still take precedence)',
    '关闭 / 只观察（记录但不拦） / 强制执行': 'Off / observe only (record but never block) / enforce',
    '地区库来自 MaxMind GeoLite2，不随产品分发；本产品包含 GeoLite2 数据，来源：': 'The region database comes from MaxMind GeoLite2 and is not distributed with the product; this product includes GeoLite2 data created by MaxMind, available from ',
    '我确认：启用后我自己的来源也会被拒绝，我会用服务器上的': 'I understand that after enabling, my own source will be denied too, and I can recover with',
    '或': 'or',
    '默认关闭，安装与升级都不会自动开始拦截。开启后：清单外的国家/地区返回 403；': 'Off by default — installing or upgrading never starts blocking. When enabled, countries outside the list receive 403; ',
    '无法定位的来源（含内网地址）按「未知来源」处理': 'unlocatable sources (including private addresses) follow the "unknown source" rule',
    '。地区库来自 MaxMind GeoLite2，不随产品分发；本产品包含 GeoLite2 数据，来源：': '. The region database comes from MaxMind GeoLite2 and is not distributed with the product; this product includes GeoLite2 data created by MaxMind, available from ',
    '未配置 License Key，自动更新已停用': 'No license key configured; automatic updates are disabled',
    '未配置 Account ID': 'No account ID configured',
    '未配置 Account ID：内置更新不会联网。': 'No account ID configured: the built-in updater will not connect.',
    '今日尝试': 'Attempts today',
    '暂时不能下载：': 'Download not available right now: ',
    '未配置 License Key：不会联网下载，也不会自动更新（已存在的地区库照常使用）。': 'No license key configured: no downloads and no automatic updates (an existing region database keeps working).',
    '环境变量 GEO_ENFORCE_DISABLED=true 正在强制关闭地域限制（设置里的档位不生效）。': 'The environment variable GEO_ENFORCE_DISABLED=true forces region restriction off (the mode below has no effect).',
    '地区库已更新': 'Region database updated',
    '未保存：需要先确认自锁风险': 'Not saved: confirm the self-lockout risk first',
    '放行 ': 'Allow ', '只观察（不会拦）': 'Observe only (never blocks)',
    '无法定位国家': 'country unknown', '国家/地区 ': 'Country/region ', '（这是你当前的来源）': ' (this is your current source)',
    '预演中…': 'Previewing…', '正在检查并下载地区库…': 'Checking and downloading the region database…',
    '最近一次丢弃：': 'Last drop: ', '观测处于降级状态：': 'Observation is degraded: ',
    '写库失败': 'write failures', '请检查数据库与磁盘。': 'Check the database and disk.',
    '存在采样丢弃，记录不完整。': 'some records were dropped, so the records are incomplete.',
    '已改为不按天清理记录。': 'Age-based cleanup is now off.',
    '已开始记录访问。': 'Access recording started.',
    '已停止记录访问（已有记录保留到清理）。': 'Access recording stopped (existing records remain until cleanup).',
    '已导出当前筛选范围内的访问记录。': 'Exported the access records in the current filter range.',
    '导出内容为空，没有可下载的记录。': 'Nothing to export — no records in the current filter range.',
    '导出失败：': 'Export failed: ', '保存失败：': 'Save failed: ',
    '导出的记录达到上限，只包含最新的部分；缩小时间范围可导出全部。': 'The export hit its row cap and contains only the newest part; narrow the time range to export everything.',
    // 投屏工作台
    ...
    '今天到期': 'Expires today',
    '打开该设备的投屏页': 'Open this device in the mirror workspace',
    '未找到该设备，可能已被删除或你没有访问权限': 'Device not found — it may have been deleted, or you do not have access',
    '日志保存时长': 'Log retention', '审计日志与运行日志的保留天数': 'Retention for audit and runtime logs',
    '保存时长': 'Retention', '不清理': 'Do not clean up',
    '1 天': '1 day', '3 天': '3 days', '7 天': '7 days', '15 天': '15 days', '30 天': '30 days',
    '保留天数': 'Retention (days)', '保留最近': 'Keep the last', '永久保留': 'Keep forever',
    '读取中…': 'Loading…',
    '日志保存时长已保存': 'Log retention saved',
    '系统更新': 'System update', '检查是否有已发布的新版本': 'Check for a newer published release',
    '等待检查': 'Waiting for check', '检查更新': 'Check for updates',
    '当前版本': 'Current version', '最新版本': 'Latest version',
    '这里只显示是否有更新。应用更新在服务器上执行下面的命令（会先备份数据与 .env，只换镜像，失败自动回滚）。': 'This panel only reports availability. Apply the update on the server with the command below (it backs up the data and .env, swaps the image only, and rolls back on failure).',
    '日志保存时长只能是：不清理 / 1 / 3 / 7 / 15 / 30 天': 'Log retention must be one of: do not clean up / 1 / 3 / 7 / 15 / 30 days',
    '「不清理」仅关闭按天清理，行数与文件轮转上限仍生效。审计日志清理连续过期记录并保留校验链；运行日志只清理超期轮转段。': '"Do not clean up" disables age-based cleanup only; row and rotation limits still apply. Audit cleanup removes a continuous expired prefix while preserving the integrity chain; runtime cleanup removes expired rotated segments only.',
    ...
    '关闭提示': 'Dismiss notice',
    '控制权已被此账号的另一端接管，当前仍可观看。': 'Another client using this account has taken control. You can still watch.',
    '控制权已被 {0} 接管，当前仍可观看。': '{0} has taken control. You can still watch.',
    '当前卡片宽 {0}px': 'Card width: {0}px',
    '（实际 {0}px）': ' (actual {0}px)',
    '当前卡片宽 {0}px（受可用宽度限制，滑块请求 {1}px）': 'Card width: {0}px (limited by available width; requested {1}px)',
  ...
  var RULES_EN = [
    // 访问记录（VIS）：面板里的数值句子
    [/保留\s*(\d+)\s*天/g, 'keep $1 days'],
    [/上限\s*([\d,]+)\s*行/g, 'cap $1 rows'],
    [/访问记录保留\s*(\d+)\s*天。/g, 'Access records kept for $1 days.'],
    [/队列压力过大时停止了部分记录（累计丢弃\s*([\d,]+)\s*条），下面的统计只代表已记录的部分，不是完整总量。/g,
      'Some records were skipped under queue pressure ($1 dropped so far); the counters below cover only what was recorded, not the full total.'],
    [/明细 · (\S+)（(\d+) 条已加载）/g, 'Details · $1 ($2 loaded)'],
    // IP 封禁（BAN）：计数摘要。专有词条「生效」在别处被译成 apply，
    // 整句词条优先，避免出现「1 个apply封禁」这种半截翻译。
    [/个生效封禁/g, ' active ban(s)'],
    [/（含 (\d+) 个永久）/g, ' ($1 permanent)'],
